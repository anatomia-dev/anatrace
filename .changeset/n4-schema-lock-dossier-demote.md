...
**Wording sweep.** Genuinely-bare "no LLM" user-facing claims tightened to "zero-LLM in the published verdict path"; a precise **grep-guard CI test** (forbidding absolute "no LLM anywhere" claims, requiring every "zero-LLM" mention to be scoped) makes the sweep mechanical and forward-covers the essay. Docs: `docs/reference/coverage-record.md`.
